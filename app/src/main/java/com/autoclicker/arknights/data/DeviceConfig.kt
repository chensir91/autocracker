...
object DeviceConfig {
    ...
    /** 弹窗X按钮3个固定位置 — v3.22 密度搜索 ✅3张截图像素级验证
     *  签到X: 灰圆 X:84.2-86.7% Y:7.9-11.2% 质心(85.9%,9.5%)
     *  活动X: 灰圆 X:93.6-95.1% Y:10.9-14.3% 质心(94.4%,12.6%)
     *  公告X: 白色X 约(93-97%, 1-4%) — 原区域(86-93%)完全未覆盖!
     *  修正: 区域紧贴按钮本身(±1.5%)，避免面积过大稀释密度
     */
    val POPUP_X_AREAS = listOf(
        PctRect(83.5f, 7f, 88f, 12f),   // 签到弹窗X (紧贴灰圆范围)
        PctRect(92.5f, 10f, 96.5f, 15.5f), // 活动弹窗X (紧贴灰圆范围)
        PctRect(91.5f, 0.5f, 97.5f, 5.5f)  // 公告弹窗X (修正:原86-93%完全偏离)
    )
    ...
    /** 弹窗X按钮: 灰圆背景(RGB≈70-170) + 白色X图标(RGB≈180-220) — v3.22 ✅3张截图像素验证
     *  灰圆像素实测: RGB≈85-100 (原规则下限110漏掉了大部分!)
     *  白色X像素实测: RGB≈184-211
     *  扩大范围到60-220，保留灰度约束(差异<30)排除彩色干扰
     */
    val COLOR_POPUP_X = ColorRule("弹窗X", { it in 60..220 }, { it in 60..220 }, { it in 60..220 })
