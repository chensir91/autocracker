...
object DeviceConfig {
    ...
    /** 弹窗X按钮3个固定位置 — v3.23 排除源石/+干扰
     *  签到X: 灰圆 x:84.2-87.7% y:5.6-13.2% 质心(86.0%,9.4%)
     *  活动X: 灰圆 x:92.6-96.1% y:8.6-16.4% 质心(94.4%,12.5%) — y>9%不冲突
     *  公告X: 约(92-96%, 1-5%) — v3.22区域(91.5-97.5%)与黄票+(86-97%,2-9%)严重重叠!
     *  修正: 公告X左移到89-94%避开黄票+, 颜色规则收紧到75-130双保险
     */
    val POPUP_X_AREAS = listOf(
        PctRect(83.5f, 6f, 88.5f, 13f),     // 签到弹窗X (y从7→6, 覆盖灰圆顶部)
        PctRect(92f, 9.5f, 97f, 16f),        // 活动弹窗X (y从10→9.5)
        PctRect(89f, 1f, 95f, 6f)            // 公告弹窗X (左移避开黄票+ 86-97%)
    )
    
    /** 弹窗深色背景检测区域（用于验证X按钮确实在弹窗上） */
    val POPUP_DARK_CHECK_AREA = PctRect(20f, 30f, 80f, 70f)
    
    /** 源石/+号排除区域 — v3.23 主界面右上角资源栏，避免X搜索误匹配
     *  源石(8545+): x:73-85%, y:2-9%
     *  黄票(36+): x:86-97%, y:2-9%
     *  合并: x:72-98%, y:1-10%
     */
    val ORIGIN_STONE_EXCLUDE = PctRect(72f, 1f, 98f, 10f)
    ...
    /** 弹窗X按钮灰圆: RGB 75-130 均匀灰色 — v3.23 ✅排除源石/+干扰
     *  灰圆实测: RGB≈(88,88,88) 三通道极接近
     *  源石区蓝色: B≈126>130 被排除; 白色: >130 被排除
     *  不再用60-220宽范围(会匹配蓝色B通道和白色)
     */
    val COLOR_POPUP_X = ColorRule("弹窗X灰圆", { it in 75..130 }, { it in 75..130 }, { it in 75..130 })
